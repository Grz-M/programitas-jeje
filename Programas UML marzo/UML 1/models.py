...
class Usuario(Persona):
    ...
    def crearReserva(self,funcion,asientos):
        for x in asientos:
            if x in funcion.asientos_ocupados:
                print(f"Error: El asiento {x} ya está ocupado")
                return
        funcion.asientos_ocupados.extend(asientos)
        reserva = {"Pelicula": funcion.pelicula.titulo, "Asientos": asientos}
        self.historialReservas.append(reserva)
        print(f"Reserva creada con éxito para {self.nombre}. Asientos: {', '.join(asientos)}")
...
class Reserva:
    ...
    def aplicarPromocion(self,promo:Promocion):
        descuento=self.montoTotal*(promo.porcentajeDescuento/100)
